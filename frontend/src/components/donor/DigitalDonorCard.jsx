  ...
  return (
    <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-slate-900 via-slate-800 to-blood-950 text-white p-6 sm:p-7 shadow-2xl border border-white/10">
      ...
      <div className="flex items-center justify-between relative z-10 border-b border-white/10 pb-4 mb-5">
        ...
        <div className={`flex items-center space-x-1 backdrop-blur-md px-3 py-1 rounded-full border ${
          user.is_verified
            ? 'bg-emerald-500/10 border-emerald-500/30 text-emerald-300'
            : 'bg-amber-500/10 border-amber-500/30 text-amber-300'
        }`}>
          <ShieldCheck className={`w-3.5 h-3.5 ${user.is_verified ? 'text-emerald-400' : 'text-amber-400'}`} />
          <span className="text-[11px] font-bold">
            {user.is_verified ? 'Terverifikasi' : 'Menunggu ACC'}
          </span>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between gap-4 relative z-10 my-4">
        <div className="space-y-1">
          ...
          <p className="text-xs text-slate-300 font-mono tracking-wider">{user.donor_card_no || '-'}</p>
        </div>
      ...
      <div className="flex items-center justify-between pt-4 mt-5 border-t border-white/10 relative z-10 text-xs">
        <div className="flex items-center space-x-1.5 text-slate-300" title={user.address || user.city || '-'}>
          <MapPin className="w-4 h-4 text-rose-400 flex-shrink-0" />
          <span className="font-medium text-xs truncate max-w-[170px] sm:max-w-xs">
